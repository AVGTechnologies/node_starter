...
module NodeStarter
  ...
  class Starter
    attr_reader :job_id, :dir, :pid

    def initialize(job_id, config_values, enqueue_data)
      @job_id = job_id
      config_values[:id] = job_id
      config_values[:base_address] ||= NodeStarter.config.uss_node[:base_address] + job_id
      @config_values = NodeStarter.config.uss_node.merge(config_values)
      @enqueue_data = enqueue_data
    end

    def spawn_process
      @dir = Dir.mktmpdir("running_uss_node_#{job_id}")
